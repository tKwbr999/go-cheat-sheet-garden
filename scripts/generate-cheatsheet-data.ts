...
function parseFrontMatter(content: string): { data: ParsedFrontMatter; content: string } {
  ...
  if (tagsMatch && tagsMatch[1]) {
    ...
    if (tagsString) {
      data.tags = tagsString.split(',').map(tag => {
...
async function generateCheatSheetData() {
  ...
  for (const filePath of files) {
    ...
    const tags = frontMatter.tags ? [...new Set([...defaultTags, ...frontMatter.tags])] : defaultTags;

    const codeExample: InternalCodeExample = {
      title: frontMatter.title, description, code, tags, filePath,
    };

    if (!tempSections[chapterId]) {
      const sectionTitle = chapterId.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
      tempSections[chapterId] = {
        id: chapterId, title: sectionTitle, orderPrefix: chapterPrefix, codeExamples: [],
      };
    }
  ...
  if (!fs.existsSync(sectionsOutputDir)) {
   ...
   if (!fs.existsSync(outputBaseDir)) { // ベースディレクトリも確認
  ...
  for (const section of sortedSections) {
    ...
    // セクション別JSONファイルの書き出し
    fs.writeFileSync(sectionFilePath, JSON.stringify(finalSectionData, null, 2));
  ...
  console.log(`[generate-data] Successfully generated index file: ${indexOutputFile}`);
